...
# Convert Variable to a factor with levels ordered by decreasing concentration
v50_data$Variable <- factor(v50_data$Variable, levels = v50_data$Variable)

# Create a bar plot
ggplot(v50_data, aes(x = Variable, y = V50)) +
    geom_bar(stat = "identity", fill = "blue", width = 0.6) +
    labs(title = "Melting Points depending on Maltose Concentration",
         x = "Maltose Concentration (µM)",
         y = "V50 (Temperature)") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    coord_cartesian(ylim = c(48, NA))  # Adjust the y-axis viewable area
